...
(defcustom agent-shell-to-go-discord-reaction-map
  '((hide . ("see_no_evil" "no_bell"))
    ...
    (heart . ("heart" "heart_eyes" "heartpulse" "sparkling_heart" "revolving_hearts" "two_hearts"))
    (bookmark . ("bookmark")))
...
(defun agent-shell-to-go--discord-emoji-to-action (emoji)
  ...
  (car
   (seq-find (lambda (pair) (member emoji (cdr pair)))
             agent-shell-to-go-discord-reaction-map)))
...
Returns the parsed JSON response or nil."
  (let* ((url (concat agent-shell-to-go--discord-api-base endpoint))
    ...
    (when data
      (setq args
            (append args
                    (list "-d" (encode-coding-string (json-encode data) 'utf-8)))))
...
(defun agent-shell-to-go--discord-api-upload (channel path &optional comment)
  ...
  (let* ((url (format "%s/channels/%s/messages"
                      agent-shell-to-go--discord-api-base channel))
         (token agent-shell-to-go-discord-bot-token)
...
(defun agent-shell-to-go--discord-truncate-content (text)
  ...
  (if (> (length text) agent-shell-to-go--discord-max-content-length)
      (concat (substring text 0 (- agent-shell-to-go--discord-max-content-length 20))
              "\n_... truncated_")
    text))
  ...
  (let ((pairs
    ...
    (dolist (pair pairs)
      (pcase (car pair)
        ...
        ("DISCORD_GUILD_ID"
         (unless agent-shell-to-go-discord-guild-id
           (setq agent-shell-to-go-discord-guild-id (cdr pair))))
        ("DISCORD_CHANNEL_ID"
         (unless agent-shell-to-go-discord-channel-id
...
(defun agent-shell-to-go--discord-find-channel-by-name (name)
  ...
  (when agent-shell-to-go-discord-guild-id
    (let* ((resp
           ...
           (channels (when (vectorp resp) (append resp nil))))
      (alist-get
       'id (seq-find (lambda (ch)
                       (and (equal (alist-get 'name ch) name)
                            (= (or (alist-get 'type ch) -1) 15)))
                     channels)))))
...
(defun agent-shell-to-go--discord-create-channel (name)
  ...
  (when agent-shell-to-go-discord-guild-id
    (let* ((resp
            (agent-shell-to-go--discord-api
             "POST"
             (format "/guilds/%s/channels" agent-shell-to-go-discord-guild-id)
             `((name . ,name) (type . 15))))
...
(defun agent-shell-to-go--discord-get-or-create-project-channel (transport project-path)
  ...
  (if (not agent-shell-to-go-discord-per-project-channels)
    ...
    (let ((cached
           (gethash project-path
                    (agent-shell-to-go-discord-transport-project-channels transport))))
      (or cached
          (let* ((project-name
                  (file-name-nondirectory (directory-file-name project-path)))
                 (channel-name
                  (concat agent-shell-to-go-discord-channel-prefix
            ...
            (when channel-id
              (puthash project-path channel-id
                       (agent-shell-to-go-discord-transport-project-channels transport))
              (agent-shell-to-go--discord-save-channels transport)
              (agent-shell-to-go--debug "discord channel %s for %s"
                                        channel-name project-path))
  ...
  (let ((ws
         (or (agent-shell-to-go-discord-transport-ws transport)
             (agent-shell-to-go--ws-make
              :name 'discord
              :url-fn (lambda () agent-shell-to-go--discord-gateway-url)
              :on-frame
              (lambda (frame)
                (agent-shell-to-go--discord-handle-frame transport frame))
              :on-close
              (lambda ()
...
(cl-defmethod agent-shell-to-go-transport-send-text
    ((transport agent-shell-to-go-discord-transport)
     channel thread-id text &optional options)
...
(cl-defmethod agent-shell-to-go-transport-edit-message
  ...
  (let* ((safe (agent-shell-to-go--discord-truncate-content text))
         (resp
          (agent-shell-to-go--discord-api
           "PATCH"
           (format "/channels/%s/messages/%s" channel message-id)
           `((content . ,safe)))))
    (alist-get 'id resp)))

(cl-defmethod agent-shell-to-go-transport-upload-file
    ((transport agent-shell-to-go-discord-transport)
     channel thread-id path &optional comment)
...
(cl-defmethod agent-shell-to-go-transport-acknowledge-interaction
    ((transport agent-shell-to-go-discord-transport) interaction-token &optional options)
...
INTERACTION-TOKEN must be \"{interaction-id}:{token}\"."
  (when (and interaction-token (string-match-p ":" interaction-token))
    (let* ((colon (string-search ":" interaction-token))
           ...
           (response-type (if (plist-get options :deferred) 5 4)))
      (agent-shell-to-go--discord-api
       "POST"
       (format "/interactions/%s/%s/callback" interaction-id token)
       `((type . ,response-type))))))
...
In Discord the thread IS a channel, so THREAD-ID is the channel to query."
  (let* ((target (or thread-id channel))
         ...
         (messages (when (vectorp resp) (append resp nil))))
    (mapcar (lambda (msg)
              (list :msg-id (alist-get 'id msg)
...
CHANNEL must be a forum channel (type 15); LABEL becomes the post title."
  (let* ((post-name (if (> (length label) 100)
                        (concat (substring label 0 97) "…")
                      label))
         (opening (format "🤖 **Agent Shell Session** @ %s\n_%s_"
                          (system-name)
  ...
  (let (result)
    ...
    (when agent-shell-to-go-discord-guild-id
      (let* ((resp
             ...
             (threads (when resp (append (alist-get 'threads resp) nil))))
...
(cl-defmethod agent-shell-to-go-transport-delete-thread
  ...
  (when thread-id
    (agent-shell-to-go--discord-api
     "DELETE" (format "/channels/%s" thread-id))))
          ...
          (websocket-send-text
           ...
           (json-encode
            `((op . ,agent-shell-to-go--discord-op-heartbeat)
              (d . ,(or (agent-shell-to-go-discord-transport-sequence transport)
                        :null)))))
        (error
...
(defun agent-shell-to-go--discord-start-heartbeat (transport interval-ms)
  ...
  (let ((interval-sec (/ interval-ms 1000.0)))
    (setf (agent-shell-to-go-discord-transport-heartbeat-timer transport)
          (run-with-timer interval-sec interval-sec
...
(defun agent-shell-to-go--discord-send-identify (transport)
  ...
  (let ((ws (agent-shell-to-go-discord-transport-ws transport)))
    (when (agent-shell-to-go--ws-connected-p ws)
      (let* ((intents
              (logior agent-shell-to-go--discord-intent-guilds
                      ...
                      (if agent-shell-to-go-discord-message-content-intent
                        ...
                        0)))
             (payload `((op . ,agent-shell-to-go--discord-op-identify)
                        (d . ((token . ,agent-shell-to-go-discord-bot-token)
                              (intents . ,intents)
                              (properties . ((os . "linux")
                                             (browser . "agent-shell-to-go")
                                             (device . "agent-shell-to-go"))))))))
        (websocket-send-text (agent-shell-to-go--ws-websocket ws)
                             (json-encode payload))))))
      ...
      (let* ((op (alist-get 'op data))
        ...
        (cond
         ...
         ((= op agent-shell-to-go--discord-op-invalid-session)
          (agent-shell-to-go--debug "discord gateway: invalid session, re-identifying in 5s")
...
(defun agent-shell-to-go--discord-dispatch-event (transport event-type data)
  ...
  (pcase event-type
    ...
    ("MESSAGE_CREATE"
     (agent-shell-to-go--discord-normalize-message transport data))
  ...
  (let* ((msg-id (alist-get 'id data))
    ...
    (unless (or bot-p
      ...
      (when (agent-shell-to-go-transport-authorized-p transport user-id)
        (let* ((resolved (agent-shell-to-go--discord-resolve-channel transport channel-id))
...
ADDED-P is t for MESSAGE_REACTION_ADD, nil for MESSAGE_REACTION_REMOVE."
  (let* ((channel-id (alist-get 'channel_id data))
    ...
    (unless (equal user-id bot-uid)
      (agent-shell-to-go--debug "discord reaction: %s on %s (%s)"
                                emoji-name msg-id (if added-p "added" "removed"))
      (when (agent-shell-to-go-transport-authorized-p transport user-id)
        (let* ((resolved (agent-shell-to-go--discord-resolve-channel transport channel-id))
               (hook-channel (car resolved)))
          (apply #'run-hook-with-args
...
(defun agent-shell-to-go--discord-normalize-interaction (transport data)
  ...
  (let* ((interaction-id (alist-get 'id data))
         ...
         (command-name (when cmd-data (concat "/" (alist-get 'name cmd-data))))
         (options (when cmd-data (append (alist-get 'options cmd-data) nil)))
    ...
    (when (= (or interaction-type 0) 2)
      ...
      (when (agent-shell-to-go-transport-authorized-p transport user-id)
        (let* ((args-text
               ...
               (resolved (agent-shell-to-go--discord-resolve-channel transport channel-id))
               (hook-channel (car resolved)))
          (apply #'run-hook-with-args
...
Must be called once after initial bot setup or command changes."
  ...
  (let* ((gid (or guild-id agent-shell-to-go-discord-guild-id))
         ...
         (commands
          `[((name . "new-agent")
             (description . "Start a new agent-shell session")
             (type . 1)
             (options . [((name . "folder")
            ...
            ((name . "new-agent-container")
             (description . "Start a new containerized agent-shell session")
             (type . 1)
             (options . [((name . "folder")
                          (description . "Working directory (optional)")
                          (type . 3)
                          (required . :json-false))]))
            ((name . "new-project")
             (description . "Create a new project and start an agent")
             (type . 1)
